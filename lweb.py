...
app.layout = html.Div([
    ...
    html.Br(),
    html.Br(),
    dcc.Input(id = 'o', value = 'Av Desarrollo 1801, Complejo Industrial Impulso, 31183 Chihuahua, Chih., Mexico', placeholder="Origin address", type = 'text', persistence=False),
    dcc.Input(id = 'd', value = 'Blvd. Adolfo López Mateos 909 Ote. Esq. con, Av Tamaulipas, Valle de Aguayo, 87020 Cd Victoria, Tamps., Mexico', placeholder="Destination address", type = 'text', persistence=False),
    dcc.Input(id = 'tonnage', placeholder="Tonnage (t)", type = 'number', persistence=False),
    html.Button('Register', id='btn-nclicks-1', n_clicks=0),
    ], id="element-to-hide"),
    html.Br(),
    html.Br(),
    dcc.Input(id = 'w_s', value = '67806 Linares, Nuevo Leon, Mexico', placeholder="Cargo pickup", type = 'text'),
    dcc.Input(id = 'w_e', value = 'Blvd Harold R. Pape 1202, La Loma, 25770 Monclova, Coah., Mexico', placeholder="Cargo destination", type = 'text', persistence=False),
    dcc.Input(id = 'tonnage2', placeholder="Tonnage (t)", type = 'number', persistence=False),
...
def update_output(btn1, btn2, o, d, w_s, w_e, tonnage, tonnage2):
  if "btn-nclicks-1" == ctx.triggered_id:
    ...
    fig_map1.update_layout(margin ={'l':10,'t':10,'b':10,'r':10}, mapbox = {'center': {'lon': lon1, 'lat': lat1}, 'style': "open-street-map", 'zoom': 6},height=700)
